...
#define PORT_1_MEM_BASE 0x40080
	...
	while(1)
	{
		...
		bzero(buf,N_BUF);
	   	...
	   	{   
			...
			peripheral_write8(dualPortRam, CONTROL_ADDR, 0x0);
			printf("Writing numbers...\n");
			peripheral_write8(dualPortRam, NUMBERS_ADDR, ((B & 0xF) << 4)| (A & 0xF));
			printf("Set control bit\n");
			peripheral_write8(dualPortRam, CONTROL_ADDR, 0x1);

			while(!peripheral_read8(dualPortRam,STATUS_ADDR));

			result = peripheral_read8(dualPortRam, RESULT_ADDR);
			...
			{
				usleep(100);
				mem_read =  peripheral_read8(dualPortRam,i);
